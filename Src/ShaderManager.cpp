...
ShaderManager::getShaderModule(const std::string& shaderName) {
  ...
  createInfo.pCode = reinterpret_cast<const uint32_t*>(newShader.code.data());

  if (vkCreateShaderModule(
          this->_device,
          &createInfo,
          nullptr,
          &newShader.module) != VK_SUCCESS) {
