...
NdcCoord screenToNdc(const VkExtent2D& screenDims, double screenX, double screenY) {
...
InputManager::InputManager(GLFWwindow* window_) {
  ...
  this->addKeyBinding(
      ...
      std::bind([](GLFWwindow* window, InputManager* inputManager) {
...
bool InputManager::addKeyBinding(const KeyBinding& binding, std::function<void()>&& callback) {
...
bool InputManager::addMouseBinding(const MouseButtonBinding& binding, MouseButtonBindingCallback&& callback) {
  return this->_mouseButtonBindings.emplace(binding, std::move(callback)).second;
}
...
uint32_t InputManager::addMousePositionCallback(MousePositionCallback&& callback) {
  this->_mousePositionCallbacks.emplace(this->_currentMouseCallbackId, std::move(callback));
